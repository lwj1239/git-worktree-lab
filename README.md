...
基础程序和单词统计已合并到主线；本任务添加大写转换，并在任务分支同步主线、解决冲突后保留两个功能。后续评审及合并以实际执行记录为准。

当前功能：帮助信息、单词统计（示例：count "hello world" → 2）、大写转换（示例：upper "Hello" → HELLO）
...
```powershell
uv run git-worktree-lab count "hello world"
# 输出：2
```

规则：按空白分隔计数；连续空白不会产生空词，制表符和换行同样作为分隔符，空字符串及纯空白文本计为 0。包含空白的文本需作为一个参数传入。
...
测试实际调用安装后的命令入口，覆盖无参数、`--help`、`-h`、未知参数，以及单词统计的普通文本、空字符串、连续空白、制表符、换行、纯空白、混合空白和缺少文本参数；还覆盖大写转换的空字符串、混合大小写、Unicode 转换和缺失文本参数。
